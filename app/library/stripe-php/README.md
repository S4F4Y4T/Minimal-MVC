...
```php
php\lib\Stripe::setApiKey('sk_test_BQokikJOvBiI2HlWgH4olfQ2');
$charge = php\lib\Charge::create(['amount' => 2000, 'currency' => 'usd', 'source' => 'tok_189fqt2eZvKYlo2CTGBeg6Uq']);
echo $charge;
```
...
$curl = new php\lib\HttpClient\CurlClient();
...
php\lib\ApiRequestor::setHttpClient($curl);
...
```php
// set up your tweaked Curl client
$curl = new php\lib\HttpClient\CurlClient([CURLOPT_PROXY => 'proxy.local:80']);
// tell Stripe to use the tweaked client
php\lib\ApiRequestor::setHttpClient($curl);
```
...
```php
php\lib\Stripe::setLogger($logger);
```
...
```php
$charge = php\lib\Charge::create(['amount' => 2000, 'currency' => 'usd', 'source' => 'tok_visa']);
...
```php
$curl = new php\lib\HttpClient\CurlClient([CURLOPT_SSLVERSION => CURL_SSLVERSION_TLSv1]);
php\lib\ApiRequestor::setHttpClient($curl);
```
...
php\lib\Charge::retrieve("ch_18atAXCdGbJFKhCuBAa4532Z", [
    'api_key' => 'sk_test_...',
    'stripe_account' => 'acct_...'
]);
```
...
```php
php\lib\Stripe::setCABundlePath("path/to/ca/bundle");
```
...
```php
php\lib\Stripe::setMaxNetworkRetries(2);
```
...
```php
php\lib\Stripe::setEnableTelemetry(false);
```
...
```php
php\lib\Stripe::setAppInfo("MyAwesomePlugin", "1.2.34", "https://myawesomeplugin.info");
```
